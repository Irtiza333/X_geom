...
def main():
    ...
    r_expect = blade.r_root * blade.d / 2.0        # extended root radius
    all_ok &= check("root ring at (extended) r_root",
                    np.allclose(rr, r_expect, atol=1e-6),
                    f"radius range [{rr.min():.6f}, {rr.max():.6f}], "
                    f"expected {r_expect:.6f} "
                    f"(design root {blade.r_root_design * blade.d / 2:.6f})")
